...
contract Vault is ERC4626, Auth {
    ...
    /// @param _UNDERLYING The ERC20 compliant token the Vault should accept. Do you provide an address for this or what?
    constructor(
        ERC20 _UNDERLYING
    )
        ERC4626(
        ...
        Auth(Auth(msg.sender).owner(), Auth(msg.sender).authority())
    {
        ...
        // totalSupply of rvTokens = type(uint256).max
    ...
    /// The vault has to keep some yield of the profit earned from executing strategy for itself. This section determines
    /// how much percentage of the profit(yield) earned the vault is to keep for itself.

    /// @notice The percentage of profit recognized in each harvest to reserve as fees.
    ...
    /// This section determined when to withdraw funds from the strategy.
    ...
    /// @notice The period in seconds over which locked profit in the vault is unlocked.
    ...
    /// @param newTargetFloatPercent The new target float percentage.
    function setTargetFloatPercent(
        uint256 newTargetFloatPercent
    ...
    /// @notice Emitted when whether the Vault should treat the underlying token as WETH is updated.
    /// @param user The authorized user who triggered the update.
    ...
    /// @notice Sets whether the Vault treats the underlying token as WETH.
    ...
    function exchangeRate() public view returns (uint256) {
        ...
        // @audit The below may lead to an inflation attack
    ...
    /// @dev Strategies that are untrusted, duplicated, or have no balance are
    /// filtered out when encountered at withdrawal time, not validated upfront.
    function setWithdrawalStack(
        Strategy[] calldata newStack
    ) external requiresAuth {
    ...
    function replaceWithdrawalStackIndexWithTip(
        uint256 index
    ) external requiresAuth {
    ...
    /// @param index1 One index involved in the swap
    /// @param index2 The other index involved in the swap.
    function swapWithdrawalStackIndexes(
        uint256 index1,
        uint256 index2
    ) external requiresAuth {
